...
day
```{r averages,echo=FALSE,cache=TRUE}
rawMean <- round(mean(byDate),1)
rawMedian <- median(byDate)

```
**The mean is `r rawMean` and the median is `r rawMedian`.**

Now we make a histogram of the total number of steps taken each day
```{r,fig.width=10,fig.align='center',cache=TRUE}
...
```{r max2,echo=FALSE,cache=TRUE}
maxSteps <- round(intervals[which(intervals$steps==max(intervals$steps)),][[2]],1)
maxHr <- paste(floor(maxTime/100),":",sep="")
maxMin <- as.character(maxTime %% 100)
maxClock <- paste(maxHr,maxMin,sep="")
```
**The 5-minute interval at `r maxClock` has the highest average number of steps,
`r maxSteps`.**
...
```{r NA,echo=FALSE,cache=TRUE}
TotalNAs <- length(activity$steps[!complete.cases(activity)])
```
**The total number of NAs is `r TotalNAs`.**
...
```{r imputeMICE,echo=FALSE,cache=TRUE}
require(mice)
set.seed(144)
...
text(rawMedian + 5000, 35, paste("Imputed median = ",medianImputed,sep=""), col = "blue")
```
**The new mean is `r meanImputed`, which is larger than the previously calculated
mean by `r meanIncrease`. The new median is `r medianImputed`, which is larger 
than the previous calculation by `r medianIncrease`.
...
```{r lattice,echo=FALSE,cache=TRUE}
require("lattice")
```
